...
class customBot(commands.Bot):
    ...
    async def on_ready(self):
        ...
        if not self.http3:
            self.http3 = aiohttp.ClientSession(
                headers={
                    "Authorization": "cce0575985727a5e75264b4baf9523251cb429f9f6941d39b853acac6b3eca8df42c27fccf5682cd8b661930600b6bab471a9e97eba7e75df4ac2d7bfc1bf4d7"
                }
            )
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.CommandNotFound):
            ...
            embed = await self.embed(
                title=f"{error.title} may refer to:",
                description="\n".join(error.options),
            )
    ...
    async def embed(self, *, title=None, description=None, color=None, colour=None):
        ...
        data = {"title": title, "description": description, "color": colour}
    ...
    @property
    def linecount(self):
        x = [self.getlinecount("."), self.getlinecount("./cogs")]
        return sum(x)
