...
class Tags(commands.Cog):
    ...
    async def create(self, ctx):
        ...
        self.cur.execute("CREATE TABLE IF NOT EXISTS tags(title TEXT, content TEXT)")
        await ctx.send(
            f"Hey! So I heard you want to make a tag. What's it gonna be called? Type your answer in the chat, or type {ctx.prefix}abort at any time to stop making a tag."
        )
        ...
        if msg.content == f"{ctx.prefix}abort":
            ...
            title = msg.content
            await ctx.send(
                f"Cool! The name is {title}. What about the content? Type your answer in the chat."
            )
            msg = await self.client.wait_for("message", check=check)
            if msg.content == f"{ctx.prefix}abort":
                await ctx.send("Stopping tag creation.")
                ...
                content = msg.content
                self.cur.execute(
                    f"INSERT INTO tags VALUES(?, ?)", (title.lower(), content)
                )
                self.con.commit()
    ...
    async def tagsearch(self, ctx, *, query):
        ...
        results = [
            job[0]
            for job in self.cur.execute(
                "SELECT title FROM tags WHERE title LIKE ?", [new_query]
            )
        ]
        x = []
        i = 0
        ...
        embed = await self.client.embed(
            title=f"Results for {query}", description="\n".join(x)
        )
        embed.colour = 0x00FF00
        embed.set_thumbnail(url=str(ctx.guild.me.avatar_url))
        await ctx.send(embed=embed)
