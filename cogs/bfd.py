...
class bfd(commands.Cog, name="Bots for Discord"):
    ...
    async def votes(self, ctx):
        async with self.client.http3.get(
            f"https://botsfordiscord.com/api/bot/{self.client.user.id}/votes"
        ) as resp:
            ...
            try:
                ...
                await ctx.send(
                    f"**{self.client.user.display_name}** has **{vote_count}** votes!"
                )
            except AssertionError:
                await ctx.send(
                    "Looks like the service is currently down. Please try again."
                )
